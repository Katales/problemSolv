...
class Bikes { 
    ...
    getNewBikeFor(order) { 
        ...
        while( this.busy.ptr < this.busy.list.length
                && order.end >= this.busy.list[this.busy.ptr] ) 
            this.busy.ptr++;
...
function maxNumberOfBikes() {
    const bikes = new Bikes();

    orders.forEach( (order) => {
        bikes.clearBusyPtr();
        while (order.begin >= bikes.busy.list[bikes.busy.ptr])
            bikes.busy.ptr++;
        bikes.releaseBike();
...
console.log("Max number of bikes to process all orders is :", maxNumberOfBikes());
